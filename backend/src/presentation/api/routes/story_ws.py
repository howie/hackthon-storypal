...
async def story_websocket(
    ...
    logger.info(
        "[StoryWS] LLM provider selected: %s (available: %s)",
    ...
    tts_provider = None
    try:
        tts_provider = TTSProviderFactory.create_default("gemini-pro")
        logger.info("[StoryWS] TTS provider: gemini-pro")
    except Exception:
        logger.warning("Gemini TTS provider unavailable, story will be text-only")

    def _repo_factory() -> StoryRepositoryImpl:
        """Create a new repository with its own DB session for fire-and-forget ops."""
        return StoryRepositoryImpl(AsyncSessionLocal())
