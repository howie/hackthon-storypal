...
### 2. Imagen 4 — 16×16 LED 像素畫生成

StoryPal 的實體裝置搭載 16×16 LED 像素矩陣來顯示故事插圖。在如此極端的低解析度下產生可辨識的圖像，需要大量的 prompt engineering：

- **16×16 像素限制**：每張插圖必須在 16×16 LED 網格上清晰可辨 — 總共只有 256 個像素。我們測試了大量 prompt 策略來產生乾淨、高對比的像素畫，使其在此解析度下仍可辨識（如「16x16 pixel art, single subject, minimal detail, bold colors, black outline」）。
- **風格一致性**：我們在每個圖片 prompt 前加上統一的像素畫風格前綴，以維持故事場景間一致的視覺語言，但在 16×16 下維持一致的角色表現仍是開放的挑戰。
- **仍有改善空間**：儘管經過大量 prompt 迭代，結果仍不穩定 — 部分主題渲染效果良好，但其他主題在 16×16 下會變成無法辨認的色塊。更好的 prompt 模板、微調模型、或後處理縮圖流程是潛在的改進方向。
- **安全防護**：Imagen 4 內建的安全過濾器與兒童內容需求高度契合。正面描述（「友善的龍」）比否定描述（「不可怕的龍」）效果更好。
