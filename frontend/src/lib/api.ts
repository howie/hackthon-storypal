...
/**
 * Resolve a backend file URL (e.g. `/files/previews/...`) to an absolute URL.
 *
 * In production, frontend and backend live on different origins.
 * The backend mounts `/files/` at its root, so we strip `/api/v1` from the
 * API base URL to get the backend origin, then prepend it to relative paths.
 * Absolute URLs (e.g. ElevenLabs CDN) are returned as-is.
 */
export function resolveBackendFileUrl(url: string): string {
  if (url.startsWith('http://') || url.startsWith('https://')) {
    return url
  }
  const backendOrigin = API_BASE_URL.replace(/\/api\/v\d+\/?$/, '')
