...
import { api, resolveBackendFileUrl } from '../lib/api'
import type {
  ConversationTurn,
  InteractionMode,
  InteractionSession,
 ...
 */
export function buildWebSocketUrl(mode: InteractionMode, token: string): string {
  // Try dedicated WS URL first, then derive from API URL
  const wsUrl = getWsUrl()
  if (wsUrl) {
    return `${wsUrl}/api/v1/interaction/ws/${mode}?token=${token}`
  }
  ...
  return `${wsProtocol}://${host}/api/v1/interaction/ws/${mode}?token=${token}`
}
...
export function getAudioFileUrl(audioPath: string): string {
  return resolveBackendFileUrl(`/files/${audioPath}`)
}
