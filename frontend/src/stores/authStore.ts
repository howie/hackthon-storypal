  ...
  persist(
    (set, get) => ({
      ...
      login: async () => {
        ...
        // Navigate directly to backend OAuth endpoint (NOT through Vite proxy).
        // The proxy cannot handle 307 redirects to external domains (accounts.google.com).
        // In production, VITE_API_BASE_URL is a full URL so this works naturally.
        // In local dev, VITE_OAUTH_BASE_URL provides the direct backend address.
        ...
        const apiBaseUrl = runtimeConfig?.VITE_API_BASE_URL
          ...
          || '/api/v1'
        // For OAuth, use VITE_OAUTH_BASE_URL (direct backend) to avoid proxy redirect issues
        const backendUrl = import.meta.env.VITE_OAUTH_BASE_URL || apiBaseUrl
