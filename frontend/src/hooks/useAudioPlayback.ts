...
/** AudioContext with setSinkId support (Chrome 110+) */
...
export interface UseAudioPlaybackOptions {
  ...
  volume?: number
  /** Output device ID for audio playback (Chrome 110+, ignored on unsupported browsers) */
  sinkId?: string
...
export function useAudioPlayback(options: UseAudioPlaybackOptions = {}): UseAudioPlaybackReturn {
  ...
  const getAudioContext = useCallback(() => {
    if (!audioContextRef.current) {
      ...
      if (sinkId && 'setSinkId' in audioContextRef.current) {
        ;(audioContextRef.current as AudioContextWithSinkId).setSinkId(sinkId).catch((err) => {
          console.warn('Failed to set initial audio output device:', err)
  ...
  }, [sampleRate, volume, sinkId])

  // Switch output device when sinkId changes
  useEffect(() => {
    const ctx = audioContextRef.current
    if (!ctx || !sinkId) return
    if ('setSinkId' in ctx) {
      ;(ctx as AudioContextWithSinkId).setSinkId(sinkId).catch((err) => {
