...
export interface GeminiLiveConfig {
  ...
  voice: string
  systemPrompt?: string
  /** Direct API key (benchmarking only). Omit to use backend proxy with JWT token. */
  apiKey?: string
}
...
export function useGeminiLive(
  ...
  const connect = useCallback(
    (config: GeminiLiveConfig) => {
      ...
      // apiKey: legacy direct connection (benchmarking). Otherwise use JWT proxy.
      const url = config.apiKey
        ? `${config.wsUrl}?key=${config.apiKey}`
        : (() => {
            const token = localStorage.getItem('auth_token') || ''
            return token ? `${config.wsUrl}?token=${token}` : config.wsUrl
          })()
