...
interface InteractionPanelProps {
  /** User ID for the session (used for session metadata) */
  userId: string
...
export function InteractionPanel({ userId, token, wsUrl, className = '' }: InteractionPanelProps) {
  ...
  // Determine WebSocket URL (authenticated via JWT token)
  // Use buildWebSocketUrl to get the correct backend API URL (not frontend domain)
  const resolvedWsUrl = wsUrl || buildWebSocketUrl(options.mode, token)
