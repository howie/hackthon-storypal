...
export function StoryPlayer({ onExit, onStartReview }: StoryPlayerProps) {
  const {
    session,
    playState,
    turns,
  ...
  // Connect WebSocket on mount — use getState() to avoid StrictMode double-mount
  // triggering extra WS connections due to function reference changes
  useEffect(() => {
    const { connectWS: connect, disconnectWS: disconnect } = useStoryPalStore.getState()
    connect()
    return () => {
      disconnect()
    }
  ...
  useEffect(() => {
    if (isConnected && session && playState === 'loading') {
      console.log('[StoryPlayer] sending story config', session.template_id)
      sendStoryConfig({
